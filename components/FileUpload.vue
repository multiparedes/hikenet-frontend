<template>
    <div class="flex flex-col gap-y-4" v-auto-animate>
        <div class="flex gap-x-4 w-full justify-around max-w-xs" v-auto-animate>
            <Button type="button" class="mr-auto" variant="outline" icon-start icon="fluent:cloud-add-24-regular"
                @click="openInput">{{
                    $t('upload_images') }}</Button>
            <input class="hidden" ref="imageInputRef" type="file" accept="image/jpeg,image/png,image/jpg"
                multiple="multiple" @change="uploadImage">

            <Button type="button" v-if="images.length > 0" variant="destructive" icon-start
                icon="fluent:delete-24-regular" @click="images = []">{{
            ...
            <Carousel v-if="images.length > 0" class="relative w-full max-w-xs">
                <CarouselContent>
                    <CarouselItem v-for="( blob, index ) in  images " :key="index">
                        ...
                        <p class="py-1 text-center">{{ index }} / {{ images.length }}</p>
                    </CarouselItem>
                </CarouselContent>
