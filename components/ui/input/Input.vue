...
<template>
  <div class="relative w-full max-w-sm items-center">
    <input v-model="modelValue" :type="internalType ?? type" :placeholder="placeholder ?? ''" :class="[
      cn(
    ...
    <span class="absolute start-0 inset-y-0 flex items-center justify-center px-2">
      <Icon v-if="props.icon" :name="defaultIcons.find((i) => i.name === props.icon)?.icon || props.icon
      " class="size-6 text-primary-500" />
    </span>

    <span v-if="type === 'password'"
      class="absolute end-0 inset-y-0 flex items-center justify-center px-2 cursor-pointer" @click="switchPasswordType">
      <Icon type="button" v-auto-animate :name="passwordVisible
