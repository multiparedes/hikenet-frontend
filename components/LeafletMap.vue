    ...
    <Card class="col-span-2 row-span-2 override flex flex-col gap-2">
        <div class="grid md:grid-cols-2 grid-cols-1 gap-x-4 gap-y-2 items-center">
            <Input v-model="searcher" class="w-full" icon="fluent:search-16-regular"
                :placeholder="$t('search_a_location') + '...'" name="search" @update="debouncedSearch" />

            <div class="flex gap-x-2 items-center md:justify-end justify-between">
                <p>{{ $t("upload_hike_map") }}</p>
                <Button type="button" size="sm" variant="outline" icon="fluent:arrow-upload-16-regular">{{ $t("uplaod")
                    }}</Button>
            </div>
        </div>

        <div id="map" class="h-[35em] rounded-md overflow-clip relative">
            <Button type="button" :icon="editing ? 'fluent:hand-draw-16-regular' : 'fluent:pin-12-regular'
                " class="absolute bottom-1 left-1 z-10" :variant="editing ? 'outline' : 'default'"
                @click.stop="editing = !editing">{{ editing ? $t("move") : $t("edit") }}</Button>
        </div>
        <div ref="popupContent" :style="{ display: popupVisible ? 'block ' : 'none' }">
            <p class="text-center">
                {{ activePopoupContent }}
            </p>
        ...
        <Modal v-model="editMarkerModal" :title="$t('edit_marker')" :actions="false">
            <FormWrapper :initial="editForm" :validation="validationSchema" @submit="updateMarker">
                <TextAreaWrapper name="description" />
                <div class="mt-4 flex ml-auto gap-2 justify-end">
                    <Button variant="destructive" @click="editMarkerModal = !editMarkerModal">{{ $t("close") }}</Button>
