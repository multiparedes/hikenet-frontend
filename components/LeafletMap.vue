...
import test from "/test_file.gpx?url";
...
onMounted(() => {
  ...
  // Add existing markers from props.modelValue
  props.modelValue.forEach((markerData) => {
  ...
  if (props.center) {
    var latlng = L.latLng(props.center.lat, props.center.lng);
    map.value.panTo(latlng);
  }
});

function getUserPosition() {
  if ("geolocation" in navigator && !props.center) {
