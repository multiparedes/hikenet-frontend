<template>
  <div>
    ...
    <Card class="flex flex-col items-center relative">
      ...
      <div
        class="mt-8 text-center flex flex-col gap-2 text-slate-950 dark:text-white"
      >
        ...
        <div
          class="md:flex grid grid-cols-1 gap-x-0 md:gap-x-4 gap-y-2 justify-center"
        >
          <HoverCard>
            <HoverCardTrigger as-child>
              <Button class="col-span-2" variant="outline"
                >{{ $t("friends") }}: {{ user.followers.length }}</Button
              >
            </HoverCardTrigger>
            <HoverCardContent v-if="user.followers.length > 0">
              <div class="text-center">
                <p v-for="f in user.followers">
                  {{ f.fullName }} (<span class="text-primary-700"
                    >@{{ f.username }}</span
                  >)
                </p>
              </div>
...
const isFollowing = computed(() => {
  return props.user.followers.find(
    (f: any) => f.username === useAuth().user?.username
  );
});
...
async function handleFollow() {
  const { data, error } = await api.post(
    `/friends/${isFollowing.value ? "unfollow" : "follow"}/${
      props.user.username
    }?from=${useAuth().user?.username}`
  );
