<template>
    <section class="flex gap-2 flex-col w-full content-center ">
        <FormsInputWrapper name="users" label="Search users by username" icon="fluent:search-16-regular"
            placeholder="Search for an user ..." @update="deboundedSearch">
        </FormsInputWrapper>

        <div v-auto-animate appear>
            <div v-for="user in userList"
                class="flex gap-2 items-center hover:bg-primary-200 px-2 py-1 rounded-full transition-colors my-1 cursor-pointer"
            ...
            <span class="text-center w-full" v-if="userList.length === 0 && !firstLoad">
                {{ $t('no_results') }}
            </span>
